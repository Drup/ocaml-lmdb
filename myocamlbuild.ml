(* OASIS_START *)
(* DO NOT EDIT (digest: 0dba8efd5b154389747fa6dba981ac3e) *)
...
let package_default =
  {
     MyOCamlbuildBase.lib_ocaml = [("lmdb", ["src"], [])];
     lib_c = [("lmdb", "src", [])];
     flags =
       [
          (["oasis_library_lmdb_cclib"; "link"],
            [(OASISExpr.EBool true, S [A "-cclib"; A "-llmdb"])]);
          (["oasis_library_lmdb_cclib"; "ocamlmklib"; "c"],
            [(OASISExpr.EBool true, S [A "-llmdb"])])
       ];
     includes = [("tests", ["src"])]
  }
  ;;
